...
namespace claws
{
  ...
  /// This class is usable with any functor, even though the readablity gain only really applies to lambdas
  ///
  ...
  struct inject_self
  {
    ...
    constexpr inject_self(functor_type const &f) noexcept(std::is_nothrow_copy_constructible_v<functor_type>)
      : functor(f)
    {}

    constexpr inject_self(functor_type &&f) noexcept(std::is_nothrow_move_constructible_v<functor_type>)
      : functor(std::move(f))
    {}
